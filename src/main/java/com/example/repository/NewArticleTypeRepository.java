...
public interface NewArticleTypeRepository extends CrudRepository<NewArticleTypeEntity,Integer> {
    @Query("SELECT nat FROM NewArticleTypeEntity AS nat WHERE nat.articleId=?1 AND nat.typeId=?2 AND nat.visible=true ")
    ...
    @Transactional
    @Modifying
    @Query("DELETE FROM NewArticleTypeEntity AS nat WHERE nat.visible=true AND nat.articleId=:articleId AND nat.typeId=:articleTypeId")
    void deleteEn(@Param("articleId") String articleId, @Param("articleTypeId") Integer articleTypeId);

    @Transactional
    @Modifying
    @Query("update NewArticleTypeEntity set updatedDate =?1  where articleId =?2 and typeId=?3")
    void updateDate(LocalDateTime updatedDate, String articleId, Integer typeId);

    @Query("SELECT nat.articleId FROM NewArticleTypeEntity AS nat WHERE nat.id=?1 ORDER BY nat.createdDate desc limit ?2 ")
    List<String> getArticleId(Integer id, Integer size);

    @Query("SELECT nat.articleId FROM NewArticleTypeEntity AS nat WHERE nat.articleId <> ?1 AND nat.typeId=?2 ORDER BY nat.createdDate limit 4")
